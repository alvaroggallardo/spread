...
# --------------------------
# Scraping Conciertos.club (Asturias)
# --------------------------
...
def get_events_conciertosclub():
    ...
    try:
        ...
        for article in articles:
            # Obtener fecha del bloque
            date_div = article.select_one("div.tit_wrap div.tit")
            date_text = date_div.get_text(strip=True) if date_div else ""
            if not date_text:
                continue

            # Limpiar palabras como Hoy, Mañana, etc.
            replacements = {
                "Hoy": "",
                "Mañana": "",
                "Pasado mañana": ""
            ...
            conciertos = article.select("ul.list > li")

            for concierto in conciertos:
                try:
                    music_event = concierto.select_one("div[itemtype='http://schema.org/MusicEvent']")
                    if not music_event:
                        continue
                    ...
                    evento_text = enlace_el.get_text(strip=True) if enlace_el else "Sin título"

                    # Estilo (para mostrarlo entre paréntesis en el título)
                    estilo_span = music_event.select_one("span.estilo")
                    estilo_text = estilo_span.get_text(strip=True) if estilo_span else ""
                    if estilo_text:
                        estilo_text = estilo_text.strip(" /")
                        evento_text = f"{evento_text} ({estilo_text})"

                    # Hora
                    time_div = music_event.select_one("div.time")
                    hora = time_div.get_text(strip=True) if time_div else ""
                    ...
                    print(f"✅ Añadido: {evento_text} - {fecha_evento.date()}")
    ...
    print(f"🎉 Total eventos Conciertos.club: {len(events)}")
    return events
